...
- transaction-quality.ts
- advice.ts
- coverage-boost-txn-n2.test.ts
- report-pdf.ts
- File Map
- Phases V3–V10 — Page verticals (independent, any order)
- coverage-boost-domain-libs.test.ts
- investments.ts
- http.test.ts
...
- `authWith()` --indirect_call--> `requireUser()`  [INFERRED]
  tests/unit/goals-routes.test.ts → lib/http.ts
- `authWith()` --indirect_call--> `requireUser()`  [INFERRED]
  tests/unit/reports-saved-route.test.ts → lib/http.ts
- `AccountsPage()` --indirect_call--> `compareTextAscending()`  [INFERRED]
  app/accounts/page.tsx → lib/accounts-page.ts
- `LoginForm()` --indirect_call--> `createClient()`  [INFERRED]
  components/LoginForm.tsx → lib/supabase/client.ts
- `HistoryChart()` --calls--> `linePath()`  [EXTRACTED]
  components/accounts/NetWorthHero.tsx → lib/chart-utils.ts
...
Cohesion: 0.06
Nodes (31): RUN, stamp, month, monthLabel, RUN, stamp, today, [year, monthNumber] (+23 more)
...
Cohesion: 0.05
Nodes (38): 10. Credit and loan month changes use the wrong sign and color, 11. Household account institution metadata can be lost, 12. Goal-reached notifications re-insert on every run, 1. Establish the product and route map, 1. Export privacy checks fail open on profile-query errors, 2. Build a source inventory and dependency map, 2. GoalWizard can create a duplicate goal after allocation failure, 3. Pay-down goals discard the entered target, and complete instantly without a linked account (+30 more)
...
Cohesion: 0.11
Nodes (25): CardCarousel(), cardUrl(), withExtraParams(), CardNetworkLogo(), DASHBOARD_VIEW_TABS, dashboardHref(), DashboardView, resolveDashboardView() (+17 more)
...
Cohesion: 0.06
Nodes (31): Correctness / reliability, FundFlow Code Review, H1. Plaid webhook verification is fail-open in the default configuration, H2. `/api/import/preview` has no size limit, no row cap, and no rate limit, H3. `/api/account` DELETE destroys the entire account with no step-up or rate limit, H4. `rate_limit_hit` is a PUBLIC-executable SECURITY DEFINER function, H5. High-severity vulnerabilities in transitive `sharp` dependency, HIGH severity (+23 more)

### Community 53 - "5. Per-page target specs"
Cohesion: 0.06
...
Cohesion: 0.07
Nodes (27): dotenv, eslint, eslint-config-next, devDependencies, dotenv, eslint, eslint-config-next, @playwright/test (+19 more)

### Community 66 - "validate_palette.js"
Cohesion: 0.12
Nodes (21): contrastRatio(), CVD_MATRICES, deltaEOklab(), hexToRgb(), KNOWN_SURFACE_EXCEPTIONS, linearRgb(), oklabFromLinear(), pairDistances() (+13 more)
...
Cohesion: 0.12
Nodes (26): assignBaseColours(), assignGroupColours(), buildColours(), COLUMN_X_FRACTIONS, gradientId(), GROUP_COLOURS, inheritCategoryColours(), KNOWN_GROUP_SLOTS (+18 more)

### Community 69 - "FundFlow handoff archive (2026-07-05 through 2026-08-09)"
Cohesion: 0.08
Nodes (25): Deploy later (Vercel), Every phase flag is now ON by default (2026-07-31), FundFlow handoff archive (2026-07-05 through 2026-08-09), Key facts / decisions, Latest session (2026-07-11, branch `feat/todos-roadmap`), Latest session (2026-07-23, branch `feat/remaining-must-haves`), Monarch visual-parity: every phase (V0 through V11) is done, Next session (as of 2026-07-29) (+17 more)

### Community 70 - "Architecture"
Cohesion: 0.08
Nodes (24): Aggregation Accuracy, Architecture, Data Model, Email Delivery, Email Rendering, Error Handling, Existing System, Integration Tests (+16 more)

### Community 71 - "coverage-boost-plaid-n4.test.ts"
Cohesion: 0.08
...
Cohesion: 0.10
Nodes (20): B1. DangerZone: step-up form can soft-lock with no feedback (medium, introduced), B2. ConnectBankButton: a spent link token is reused for the next connect (medium, pre-existing), B3. ConnectBankButton: stale `oauth_state_id` shows a spurious error on reload (low, pre-existing), B4. PasskeysSection: register can report failure after the passkey exists (low, introduced), B5. BanksSection: placeholder string `"shared"` stored in the id field (low, introduced), B6. BanksSection: share checkbox has no in-flight guard (low, introduced), Bugs, Frontend Review: Settings and Plaid Security Hardening (+12 more)

### Community 84 - "Phase 5: Recurring Page Implementation Plan"
Cohesion: 0.10
...
Cohesion: 0.10
Nodes (17): mockBadRequest, mockBuildDemoAccountSnapshots, mockBuildDemoDataset, mockCheckRateLimit, mockDecryptItemToken, mockErrorResponse, mockInvalidateDashboardCache, mockItemRemove (+9 more)

### Community 87 - "Session 2 — Phases 2–8 + Phase-1 remainder"
Cohesion: 0.10
Nodes (19): Bucket 1 — deferred items now shipped, Bucket 2 — new ideas shipped, Deferred summary (honest list), ⚠️ Deploy checklist (do these in order), Design notes, FundFlow — Roadmap Implementation Drop (2026-07-23), Phase 1 remainder, Phase 2 — Resilience & ops (+11 more)

### Community 88 - "Dashboard drill-down design"
Cohesion: 0.10
...
Cohesion: 0.16
Nodes (15): hasFourDigits(), isDigit(), isLetter(), isWhitespace(), stripTrailingAccountMask(), COLORS, generateWeeklyReportPdf(), PAGE (+7 more)

### Community 93 - "File Map"
Cohesion: 0.12
Nodes (16): File Map, Global Constraints, Later Phase Rule, PR 72 Phase 4 Budget Remediation Implementation Plan, Self-Review Checklist, Task 10: Release, Documentation, and Final Gates, Task 1: Restore a Phase 4-Only Diff, Task 2: Correct the Canonical Projection Loader (+8 more)

### Community 94 - "Phases V3–V10 — Page verticals (independent, any order)"
Cohesion: 0.12
Nodes (16): How to run this plan, Monarch Visual Parity — Implementation Plan, Phase V0 — Token retheme (the 60% phase), Phase V11 — Sweep, dark QA, stretch, Phase V1 — Shell restructure, Phase V2 — Shared component kit, Phases V3–V10 — Page verticals (independent, any order), Risk register (+8 more)
...
Cohesion: 0.12
Nodes (15): API contract, Budget Dashboard widget, Capture and refresh, FundFlow Deferred Features Completion Design, Image handling, Institution logos, Investments Dashboard widget, OFX and QFX import (+7 more)

### Community 99 - "report-email.ts"
Cohesion: 0.28
...
Cohesion: 0.12
Nodes (14): JWK, mockBadRequest, mockErrorResponse, mockGetItemByPlaidItemId, mockIsAllowedPushEndpoint, mockIsFeatureEnabled, mockLogError, mockRequireUser (+6 more)

### Community 102 - "coverage-boost-r3-n3.test.ts"
Cohesion: 0.12
...
Cohesion: 0.13
Nodes (14): Deliberately not changed, New-Changes — local change log, Security / correctness fixes, Session 1 (2026-07-05): review, fixes, CLAUDE.md, Session 2 (2026-07-05, later): pull from main + MFA enforcement + Google login, Session 3 (2026-07-05, later still): remaining must-haves #2–#6, Session 4 (2026-07-05): pulled PR #12 (Vercel Web Analytics) — review only, Session 5 (2026-07-05): charts, transactions ledger, in-app exports, UI polish (+6 more)

### Community 105 - "Remaining Must-Haves: Revocation Enforcement, Cron Alerts, Mobile Polish"
Cohesion: 0.13
...
Cohesion: 0.13
Nodes (14): mockBadRequest, mockCheckRateLimit, mockDecryptItemToken, mockErrorResponse, mockFetchInstitutionBranding, mockGetClientIp, mockGetItem, mockItemGet (+6 more)

### Community 110 - "FundFlow Todos Roadmap Implementation Plan"
Cohesion: 0.14
Nodes (13): File Map, FundFlow Todos Roadmap Implementation Plan, Global Constraints, Self Review, Task 1: P0 Goals Production Pass, Task 2: Roadmap Schema, Task 3: Planning Insight Engines, Task 4: Dashboard Planning Surfaces (+5 more)

### Community 111 - "URL contract (used by every task)"
Cohesion: 0.14
Nodes (13): Dashboard Drill-Down Implementation Plan, Global Constraints, Task 10: Full verification + docs, Task 1: Drill param normalization + URL builder (`lib/drilldown.ts` part 1), Task 2: `buildCategoryDrilldown` (pure aggregation), Task 3: `buildMerchantDrilldown` (pure aggregation), Task 4: Wire drilldown into `lib/dashboard.ts` + cache scope, Task 5: Link affordances in chart components (+5 more)
...
Cohesion: 0.15
Nodes (11): Charts, Commands, Conventions, Data access, graphify, Money and correctness, Plaid frugality, Rules that must not be broken (+3 more)

### Community 115 - "advice-content.ts"
Cohesion: 0.14
...
Cohesion: 0.32
Nodes (6): planFor(), SinkingFundsSection(), remove(), resetForm(), submit(), today()

### Community 118 - "File Structure"
Cohesion: 0.15
Nodes (12): Create, File Structure, FundFlow Dashboard Command Center Implementation Plan, Global Constraints, Modify, Remove after references are migrated, Task 1: Dashboard View Contract, Task 2: Priority Rail and Compact Toolbar (+4 more)

### Community 119 - "File Structure"
Cohesion: 0.15
Nodes (12): Accounts Page And Daily Balance History Implementation Plan, File Structure, Global Constraints, Self-Review, Task 1: Create And Apply The Snapshot Migration, Task 2: Shape And Persist Daily Snapshots, Task 3: Move Manual Account Mutations Behind The Server, Task 4: Build The Accounts Page Domain Model (+4 more)

### Community 120 - "File Structure"
Cohesion: 0.15
Nodes (12): Cash Flow Page Implementation Plan, Completion Record, File Structure, Global Constraints, Self-Review Results, Task 1: Build The Cash Flow Domain, Task 2: Load A Bounded Canonical Projection, Task 3: Add Accessible Cash Flow Visuals (+4 more)

### Community 121 - "File Structure"
Cohesion: 0.15
Nodes (12): Create, File Structure, Global Constraints, Modify, Pull Request Acceptance Checklist, Task 1: Centralize the ledger query contract, Task 2: Project displayed rows, rule-aware filters, sort comparators, and filter options, Task 3: Add bounded chunk loading and integrate complete-result pagination (+4 more)

### Community 122 - "scripts"
Cohesion: 0.15
Nodes (13): scripts, backfill:institution-logos, build, dev, lint, start, test, test:coverage (+5 more)

### Community 123 - "coverage-boost-plaid-n1.test.ts"
Cohesion: 0.15
...
Cohesion: 0.38
Nodes (7): ConnectBankButton(), ReconnectBankButton(), clearResume(), loadResume(), PlaidResume, saveResume(), { fetchMock, refreshMock, usePlaidLinkMock, stateSetters }

### Community 128 - "FundFlow completed work archive"
Cohesion: 0.17
Nodes (7): ~~Added 2026-07-31 (UI review) — three pre-existing E2E failures~~, Completed 2026-08-09 and 2026-08-10, Completed 2026-08-09: shipped-defect follow-up, Completed 2026-08-10: manifest redirect and service-worker cache policy, Completed program: Monarch visual parity (started 2026-08-02), ~~Dark-mode categorical palette fails all-pairs CVD (found 2026-07-31)~~ Fixed 2026-08-09, FundFlow completed work archive

### Community 129 - "Global Constraints"
Cohesion: 0.17
Nodes (11): Global Constraints, Remaining Must-Haves Implementation Plan, Task 1: Extract the session-id decode into a pure helper, Task 2: Revocation lookup helper, Task 3: Enforce revocation on page renders in proxy.ts, Task 4: Cron alert email helper, Task 5: Wire alerts into both cron routes, Task 6: Mobile ledger card list (+3 more)

### Community 130 - "Recurring sinking funds"
Cohesion: 0.17
Nodes (11): Data contract, Debt payoff planner, FundFlow Debt and Sinking Funds Design, Page behavior, Recurrence semantics, Recurring sinking funds, Schema, Single source of truth (+3 more)

### Community 131 - "benchmark-provider.ts"
Cohesion: 0.26
Nodes (9): BenchmarkClose, BenchmarkKey, BenchmarkProvider, BenchmarkSeriesResult, cache, cacheKey(), clearBenchmarkCache(), getCachedBenchmarkSeries() (+1 more)

### Community 132 - "ical.ts"
Cohesion: 0.30
...
Cohesion: 0.33
Nodes (4): ManualAccountsSection(), saveAccount(), toggleInclusion(), updateManualAccount()

### Community 139 - "TransactionEditor"
Cohesion: 0.38
Nodes (5): round2(), toSplitRow(), TransactionEditor(), openEditor(), save()

### Community 140 - "FundFlow QA Runbook"
Cohesion: 0.18
Nodes (10): Accounts Phase 2 Release Evidence, Browser Smoke Suite, Dependency And Security Maintenance, Diagnosing "the app looks broken" reports, FundFlow QA Runbook, Mobile QA Matrix, Plaid Sandbox Browser E2E, Post-review repair pass, 2026-08-09 (+2 more)

### Community 141 - "Global Constraints"
Cohesion: 0.18
Nodes (10): Global Constraints, Task 1: Notification Preference And Delivery Schema, Task 2: Timezone-Aware Weekly Periods, Task 3: Reconciled Weekly Report Model, Task 4: Visual Email And PDF Renderers, Task 5: First-Class Notifications Page, Task 6: Idempotent Weekly Delivery And Digest Preferences, Task 7: Documentation, Artifact Inspection, And Full Verification (+2 more)

### Community 142 - "Global Constraints"
Cohesion: 0.18
Nodes (10): Deferred Features Implementation Plan, Global Constraints, Task 1: Secure receipt persistence, Task 2: Build receipt matching and image normalization, Task 3: Implement receipt APIs, Task 4: Add the receipt inbox, Task 5: Complete Dashboard widget data, Task 6: Capture and render institution logos (+2 more)

### Community 143 - "Global Constraints"
Cohesion: 0.18
Nodes (10): Account Reconciliation Implementation Plan, Global Constraints, Task 1: Add the reconciliation schema, Task 2: Implement the reconciliation math as a pure function, Task 3: Extend the audit action union, Task 4: Implement the cleared-toggle route, Task 5: Implement the statement CRUD route, Task 6: Build the reconcile page and its data loader (+2 more)

### Community 144 - "Cash Flow Sankey: Monarch parity"
Cohesion: 0.18
Nodes (10): Cash Flow Sankey: Monarch parity, Decisions, Problem, Risks, Section 1: loan disbursement classification, Section 2: readable labels, Section 3: layout, Section 4: category palette (+2 more)

### Community 145 - "FundFlow — Future Todos"
Cohesion: 0.18
...
Cohesion: 0.20
Nodes (10): 2026-08-20: production-readiness pass (branch `feat/production-readiness-2026-08`), 2026-08-21: migration import from Mint, Monarch, and YNAB, FundFlow — Session Handoff, In flight: PR #114, Sonar refactor plus its review fixes, Older sessions, Post-review repair pass, Previous delivery: a reported "web login is broken" that was never the app, Previous delivery: every approved shipped-defect phase is implemented (+2 more)

### Community 152 - "Findings"
Cohesion: 0.20
Nodes (9): F1 — MEDIUM — Multi-currency conversion engine is shipped but unwired (inert dead code), F2 — LOW — FIRE milestone wording implied a guarantee — FIXED INLINE, F3 — LOW — Two of the three "multi-format exports" are unwired (dead exports), F4 — INFO — Regex merchant rules run user-supplied patterns with no timeout, F5 — INFO — New export routes carry no rate limit (consistent with existing exports), Findings, FundFlow Production-Readiness Review — 2026-08-20, Overall posture (+1 more)

### Community 153 - "Global Constraints"
Cohesion: 0.20
Nodes (9): Global Constraints, Quality and Verification Completion Implementation Plan, Task 1: Own and test the categorical palette validator, Task 2: Apply the approved dark palette, Task 3: Establish shared deterministic browser fixtures, Task 4: Close primary-page E2E gaps, Task 5: Perform the responsive interaction pass, Task 6: Capture final visual baselines (+1 more)
...
Cohesion: 0.31
Nodes (6): buildDemoAccountSnapshots(), buildDemoDataset(), DemoDataset, MERCHANTS, mulberry32(), toInt32()

### Community 158 - "coverage-boost-charts-n2.test.ts"
Cohesion: 0.29
...
Cohesion: 0.20
Nodes (8): ACCOUNT, mockBadRequest, mockErrorResponse, mockGetClientIp, mockRequireUser, mockTrySnapshots, mockWriteAudit, svc

### Community 164 - "http-session-revocation.test.ts"
Cohesion: 0.20
Nodes (8): mockFrom, mockGetAal, mockGetSession, mockGetUser, mockMaybeSingle, mockSelect, mockSupabaseClient, mockUpsert

### Community 165 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Debt and Sinking Funds Implementation Plan, Global Constraints, Task 1: Build the debt planner data boundary, Task 2: Add the debt planning page, Task 3: Secure and extend recurring sinking funds, Task 4: Implement cadence-aware projections, Task 5: Move sinking-fund writes to routes and update the UI, Task 6: Verify and document debt and sinking funds

### Community 166 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Global Constraints, Synced Transaction Duplicate Review Implementation Plan, Task 1: Add atomic duplicate-link persistence, Task 2: Implement deterministic duplicate detection, Task 3: Implement duplicate-review APIs, Task 4: Apply exclusions to canonical finance projections, Task 5: Add duplicate review and ledger status, Task 6: Verify and document duplicate review

### Community 167 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Global Constraints, Passkeys and MFA Recovery Implementation Plan, Task 1: Remove the unsupported backup-code store, Task 2: Configure passkeys and define availability, Task 3: Add passkey sign-in with MFA step-up, Task 4: Add passkey management to Security settings, Task 5: Support multiple named TOTP recovery factors, Task 6: Verify production authentication behavior

### Community 168 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Global Constraints, Merchant Logos and Brand Enrichment Implementation Plan, Task 1: Add the merchant_logos schema, Task 2: Implement the merchant-key normalizer and logo validator, Task 3: Build the curated dataset and the backfill script, Task 4: Resolve merchant logos after rule application and wire into MerchantAvatar, Task 5: Wire resolved logos into every MerchantAvatar render site, Task 6: Full verification pass

### Community 169 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Global Constraints, Migration Import from Mint, Monarch, and YNAB Implementation Plan, Task 1: Implement the Mint CSV sniffer and normalizer, Task 2: Implement the Monarch CSV sniffer and normalizer, Task 3: Implement the YNAB export sniffer and normalizer, Task 4: Wire the three sniffers into the existing preview/commit routes, Task 5: Update the import UI to reflect the new formats and the transactions-only scope, Task 6: Full verification pass

### Community 170 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Global Constraints, Task 1: Decide the link table's uniqueness model, then add the schema, Task 2: Implement deterministic transfer-pair detection, Task 3: Implement the transfer-review API, Task 4: Wire linked transfers into the canonical netting projection, Task 5: Build the transfer-review UI, Task 6: Full verification pass, Transfer Detection and Linking Implementation Plan

### Community 171 - "FundFlow Passkeys and MFA Recovery Design"
Cohesion: 0.22
Nodes (8): API behavior, FundFlow Passkeys and MFA Recovery Design, Passkey management, Passkey sign-in, Security model, Supabase configuration, TOTP recovery factors, Verification

### Community 172 - "backup.ts"
Cohesion: 0.42
Nodes (7): BackupEnvelope, buildBackupArchive(), deriveKey(), parseKey(), readBackupArchive(), KEY, OTHER_KEY

### Community 173 - "FundFlow"
Cohesion: 0.22
...
Cohesion: 0.25
Nodes (7): Backup Restore Path Implementation Plan, Global Constraints, Task 1: Implement the restore-plan builder as a pure function, Task 2: Implement the transactional per-table restore executor, Task 3: Implement the restore route with step-up re-auth and dry-run, Task 4: Build the Settings restore UI, Task 5: Full verification pass

### Community 181 - "Global Constraints"
Cohesion: 0.25
Nodes (7): Budget Templates and Month-to-Month Copy Implementation Plan, Global Constraints, Task 1: Add the template schema and the apply/copy RPC, Task 2: Implement template CRUD in `lib/budget-page.ts`, Task 3: Implement the template and apply routes, Task 4: Build the template and copy UI, Task 5: Full verification pass

### Community 182 - "Global Constraints"
Cohesion: 0.25
Nodes (7): Global Constraints, Task 1: Define the curated tax-category set and a validator, Task 2: Build the tax-year aggregator as a pure, split-safe function, Task 3: Rebuild the tax export route on the canonical projection, Task 4: Build the tax-category tagging and yearly export UI, Task 5: Full verification pass, Tax-Ready Categorization and Export Implementation Plan

### Community 183 - "Weekly report refinement (2026-07-13)"
Cohesion: 0.25
Nodes (7): 1. Card labels carry no institution, 2. The two breakdowns look additive but are not, 3. Page balance, 4. Invisible bars, Out of scope, Testing, Weekly report refinement (2026-07-13)

### Community 184 - "FundFlow Synced Transaction Duplicate Review Design"
Cohesion: 0.25
Nodes (7): API and UI, Detection, FundFlow Synced Transaction Duplicate Review Design, Persistence, Projection behavior, Purpose, Verification

### Community 185 - "FundFlow Quality and Verification Completion Design"
Cohesion: 0.25
Nodes (7): Browser verification, Dark categorical palette, End-to-end coverage, FundFlow Quality and Verification Completion Design, Operational verification, Scope, Visual snapshots

### Community 186 - "annual.ts"
Cohesion: 0.36
...
Cohesion: 0.25
Nodes (4): month, RUN, stamp, year

### Community 189 - "DuplicateReview"
Cohesion: 0.33
...
Cohesion: 0.29
Nodes (6): Categorical chart palette, Not part of this set, Re-step with the validator, never by eye, Seven slots is a measured ceiling, not a style preference, The validator enforces two independent gates, Two standing caveats, both needing the same relief

### Community 194 - "Setup"
Cohesion: 0.29
Nodes (7): 1. Prerequisites, 2. Environment, 3. Database, 4. Supabase Auth settings, 4b. Google sign-in (optional, free), 5. Run, Setup

### Community 195 - "FundFlow E2E suite"
Cohesion: 0.29
Nodes (6): Accounts page: `accounts.spec.ts`, Authenticated golden path — `golden-path.spec.ts`, FundFlow E2E suite, Running, TODO — destructive/lifecycle specs (need a throwaway user), What's covered today (smoke, no auth required)

### Community 196 - "profile.ts"
Cohesion: 0.47
Nodes (4): ProfileFieldsPatch, ProfilePatchResult, validateOptionalText(), validateProfilePatch()

### Community 197 - "investments-export.test.ts"
Cohesion: 0.29
...
Cohesion: 0.40
Nodes (3): Panel, TableToolbar(), triggerClass()

### Community 204 - "FundFlow architecture"
Cohesion: 0.33
Nodes (5): FundFlow architecture, Key modules in `lib/`, Request path, Subsystem invariants in full, The two Supabase clients

### Community 205 - "Added 2026-08-20: owner decisions for production readiness (Phase 3)"
Cohesion: 0.33
Nodes (6): 1. Custom domain, 2. E2E CI secrets (authenticated golden path), 3. Plaid Liabilities (real card APRs), 4. VAPID keys (web push), Added 2026-08-20: owner decisions for production readiness (Phase 3), Applied migrations

### Community 206 - "package.json"
Cohesion: 0.33
Nodes (5): name, overrides, postcss, private, version

### Community 207 - "reports.spec.ts"
Cohesion: 0.33
Nodes (4): activeMonth, FLAG_ON, RUN, stamp

### Community 208 - "transactions.spec.ts"
Cohesion: 0.33
...
Cohesion: 0.50
Nodes (4): Before this is user-visible, Phase 7 Goals is implemented (flag-gated), Two security details worth knowing, What shipped

### Community 231 - "Phase 6 Reports is implemented (flag-gated)"
Cohesion: 0.67
