...
/** One day of the spending-vs-last-month comparison. */
export interface CumulativeSpendDay {
  day: number;
  /** Cumulative spend through this day, or null for a day not yet reached. */
  thisMonth: number | null;
  /** The same for the previous month, or null past that month's final day. */
  lastMonth: number | null;
}

/** Days in a `YYYY-MM` month, leap years included. Day 0 of the next month. */
function daysInMonth(month: string): number {
  const [year, monthNumber] = month.split("-").map(Number);
  return new Date(Date.UTC(year!, monthNumber!, 0)).getUTCDate();
}

/** `YYYY-MM` shifted by whole months, as pure string arithmetic. */
