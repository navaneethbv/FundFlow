"use client";

import { useState } from "react";
import ForecastChart from "@/components/forecasting/ForecastChart";
import Button from "@/components/ui/Button";
import { applyLifeEvents, parseLifeEvent, type ForecastPoint, type LifeEvent, type LifeEventType } from "@/lib/life-events";
import { formatCurrency } from "@/lib/format";

const EVENT_ITEMS: ReadonlyArray<{ type: LifeEventType; label: string }> = [
  { type: "home_purchase", label: "Home purchase" },
  { type: "child", label: "Child" },
  { type: "income_change", label: "Income change" },
  { type: "expense_change", label: "Expense change" },
  { type: "retirement", label: "Retirement" },
];

function getEventLabel(type: LifeEventType): string {
  return EVENT_ITEMS.find((item) => item.type === type)?.label ?? type;
}

interface Props {
  basePoints: ForecastPoint[];
  monthlySavings: number;
  currentNetWorth: number;
  currency: string;
  initialEvents: LifeEvent[];
}

/**
 * Life-event forecasting. Events are editable assumptions that recalibrate the
 * existing projection engine client-side; they are persisted only through the
 * authenticated life-events API and are never presented as guarantees.
 */
export default function LifeEventsPanel({
  basePoints,
  monthlySavings,
  currentNetWorth,
  currency,
  initialEvents,
}: Readonly<Props>) {
  const [events, setEvents] = useState<LifeEvent[]>(initialEvents);
  const [type, setType] = useState<LifeEventType>("home_purchase");
  const [startMonth, setStartMonth] = useState("1");
  const [amount, setAmount] = useState("");
  const [duration, setDuration] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const adjusted = applyLifeEvents(basePoints, events, monthlySavings);
  const baseEnd = basePoints.at(-1)?.base ?? 0;
  const adjustedEnd = adjusted.at(-1)?.base ?? 0;
  const supportsDuration = type === "child" || type === "income_change" || type === "expense_change";

  function resetForm() {
    setType("home_purchase");
    setStartMonth("1");
    setAmount("");
    setDuration("");
    setEditingId(null);
  }

  function editEvent(event: LifeEvent) {
    if (!event.id) return;
    setType(event.type);
    setStartMonth(String(event.startMonth));
    setAmount(event.type === "retirement" ? "" : String(event.amount));
    setDuration(
      event.durationMonths === null ? "" : String(event.durationMonths),
    );
    setEditingId(event.id);
    setError(null);
  }

  async function submitEvent() {
    setError(null);
    const parsed = parseLifeEvent({
      type,
      startMonth: Number(startMonth),
      amount: type === "retirement" ? 0 : Number(amount),
      durationMonths: supportsDuration && duration ? Number(duration) : null,
    });
    if (!parsed.ok) {
      setError(parsed.error);
      return;
    }
    setBusy(true);
    try {
      const res = await fetch("/api/forecasting/life-events", {
        method: editingId ? "PATCH" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(editingId ? { ...parsed.event, id: editingId } : parsed.event),
      });
      const json = (await res.json().catch(() => null)) as { event?: LifeEvent; error?: string } | null;
      if (!res.ok) throw new Error(json?.error ?? "Could not save the event.");
      const savedEvent = json?.event;
      if (savedEvent) {
        setEvents((current) => editingId
          ? current.map((event) => event.id === editingId ? savedEvent : event)
          : [...current, savedEvent]);
      }
      resetForm();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not save the event.");
    } finally {
      setBusy(false);
    }
  }

  async function removeEvent(id: string) {
    setError(null);
    setBusy(true);
    try {
      const res = await fetch("/api/forecasting/life-events", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id }),
      });
      if (!res.ok) throw new Error("Could not remove the event.");
      setEvents((current) => current.filter((event) => event.id !== id));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not remove the event.");
    } finally {
      setBusy(false);
    }
  }

  return (
    <>
      <PanelShell>
        <h2 className="mb-1 text-sm font-semibold uppercase tracking-wide text-muted">
          Net worth projection
        </h2>
        <p className="mb-4 text-sm text-muted">
          Base-case end: <span className="money">{formatCurrency(baseEnd, currency)}</span>{" "}
          to <span className="money font-semibold text-foreground">{formatCurrency(adjustedEnd, currency)}</span>{" "}
          after your life-event assumptions.
        </p>
        <ForecastChart points={adjusted} currentNetWorth={currentNetWorth} />
      </PanelShell>

      <PanelShell>
      <h2 className="mb-1 text-sm font-semibold uppercase tracking-wide text-muted">
        Life events
      </h2>
      <p className="mb-4 text-sm text-muted">
        Add explicit, editable assumptions such as buying a home or having a child. The projection
        above recalibrates automatically. These are assumptions, not guarantees.
      </p>

      <ul className="mb-4 space-y-2 text-sm">
        {events.map((event) => (
          <li key={event.id ?? `${event.type}-${event.startMonth}`} className="flex items-center justify-between gap-2">
            <span className="min-w-0 truncate">
              <span className="font-medium">{getEventLabel(event.type)}</span>
              <span className="text-muted"> · month {event.startMonth}{event.durationMonths ? ` · ${event.durationMonths} months` : ""}</span>
            </span>
            <span className="shrink-0 tabular-nums">
              {event.type === "retirement" ? "Stops monthly savings" : formatCurrency(event.amount, currency)}
            </span>
            {event.id ? (
              <span className="flex shrink-0 items-center gap-2">
                <button
                  type="button"
                  onClick={() => editEvent(event)}
                  className="text-xs text-muted hover:text-foreground"
                  aria-label={`Edit ${getEventLabel(event.type)} event`}
                >
                  Edit
                </button>
                <button
                  type="button"
                  onClick={() => {
                    void removeEvent(event.id as string);
                  }}
                  className="text-xs text-muted hover:text-danger"
                  aria-label={`Remove ${getEventLabel(event.type)} event`}
                >
                  Remove
                </button>
              </span>
            ) : null}
          </li>
        ))}
        {events.length === 0 && <li className="text-sm text-muted">No life events configured.</li>}
      </ul>

      <div className="rounded-field border border-panel-border bg-panel-2 p-3">
        <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
          <label className="col-span-2 block">
            <span className="text-xs text-muted">Event</span>
            <select
              value={type}
              onChange={(e) => {
                const nextType = e.target.value as LifeEventType;
                setType(nextType);
                if (nextType === "retirement") setAmount("");
                if (nextType === "home_purchase" || nextType === "retirement") {
                  setDuration("");
                }
              }}
              className="mt-1 w-full rounded-field border border-panel-border bg-panel px-2 py-1.5 text-sm"
            >
              {EVENT_ITEMS.map((item) => (
                <option key={item.type} value={item.type}>{item.label}</option>
              ))}
            </select>
          </label>
          <label className="block">
            <span className="text-xs text-muted">Start month</span>
            <input
              type="number"
              min="1"
              value={startMonth}
              onChange={(e) => {
                setStartMonth(e.target.value);
              }}
              className="mt-1 w-full rounded-field border border-panel-border bg-panel px-2 py-1.5 text-sm"
            />
          </label>
          {type === "retirement" ? (
            <p className="self-end pb-1.5 text-xs text-muted">Stops monthly savings from the start month.</p>
          ) : (
            <label className="block">
              <span className="text-xs text-muted">Amount</span>
              <input
                type="number"
                step="0.01"
                min="0.01"
                value={amount}
                onChange={(e) => {
                  setAmount(e.target.value);
                }}
                className="mt-1 w-full rounded-field border border-panel-border bg-panel px-2 py-1.5 text-sm"
              />
            </label>
          )}
          {supportsDuration ? (
            <label className="block">
              <span className="text-xs text-muted">Months (blank = forever)</span>
              <input
                type="number"
                min="1"
                value={duration}
                onChange={(e) => {
                  setDuration(e.target.value);
                }}
                className="mt-1 w-full rounded-field border border-panel-border bg-panel px-2 py-1.5 text-sm"
              />
            </label>
          ) : null}
        </div>
        <div className="mt-3 flex items-center justify-end gap-2">
          {editingId ? (
            <Button type="button" size="sm" variant="ghost" onClick={resetForm} disabled={busy}>
              Cancel
            </Button>
          ) : null}
          <Button type="button" size="sm" variant="secondary" onClick={submitEvent} loading={busy}>
            {editingId ? "Save changes" : "Add event"}
          </Button>
        </div>
      </div>

      {error && <p className="mt-2 text-sm text-red-600">{error}</p>}
      </PanelShell>
    </>
  );
}

function PanelShell({ children }: Readonly<{ children: React.ReactNode }>) {
  return <div className="min-w-0 rounded-card border border-panel-border bg-panel p-5 shadow-card">{children}</div>;
}
